  ...
  head: () => ({
    meta: [
      { title: "Storyboard — Hooke" },
      {
        name: "description",
        content:
          "Visual storyboard from blueprint scenes — camera angles, movement, shot duration, motion, inline regenerate.",
      },
      { property: "og:title", content: "Storyboard — Hooke" },
      {
        property: "og:description",
        content: "Visual storyboard with camera direction and motion per panel.",
      },
    ],
  }),
      ...
      <header className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-[12px] text-text-secondary">
            Visual scene-by-scene breakdown with camera direction and motion.
          </p>
        </div>
...
function PanelCard({
  ...
  const motionLabel = MOTION_TYPES.find((m) => m.id === panel.motion)?.label ?? panel.motion;
  const hasMotion = panel.motion !== "static";
  ...
  return (
    <div className="group flex flex-col overflow-hidden rounded-lg border border-white/[0.06] bg-base/40">
      <div className="relative aspect-video bg-base/80">
        ...
        {hasMotion && (
          <div className="absolute bottom-2 left-2 flex items-center gap-1 rounded-full bg-black/60 px-2 py-0.5 text-[9px] font-medium text-iris backdrop-blur-sm">
        ...
        {editing ? (
          <>
            ...
            <div className="mb-2 grid grid-cols-2 gap-2">
              <div className="space-y-1">
                ...
                <Select
                  value={panel.angle}
                  onValueChange={(v) => onUpdate({ angle: v as CameraAngle })}
                >
              ...
              <div className="space-y-1">
                ...
                <Select
                  ...
                  onValueChange={(v) => onUpdate({ movement: v as CameraMovement })}
                >
                  <SelectTrigger className="h-7 text-[11px]">
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="mb-2 space-y-1">
              <Label className="text-[10px]">Motion</Label>
              <Select
                value={panel.motion}
                onValueChange={(v) => onUpdate({ motion: v as MotionType })}
              >
                <SelectTrigger className="h-7 text-[11px]">
                  <SelectValue />
                </SelectTrigger>
            ...
            {hasMotion && (
              <div className="mb-2 space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] text-text-dim">{panel.motionIntensity}/10</span>
                </div>
                <Slider
                  value={[panel.motionIntensity]}
                  min={1}
                  max={10}
                  step={1}
                  onValueChange={(v) => onUpdate({ motionIntensity: v[0] })}
                />
              </div>
            )}
          ...
          <>
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              ...
              {hasMotion && (
                <span className="inline-flex items-center gap-1 rounded border border-iris/20 bg-iris/10 px-1.5 py-0.5 text-[10px] text-iris">
                  <Zap className="h-2.5 w-2.5" />
                  {motionLabel} · {panel.motionIntensity}
                </span>
              )}
            </div>
